...
@property (nonatomic, strong) NSArray         *repeatKeys;  //转化成数组的字段
...
- (void)continuousEndKey:(NSString *)key {
    ...
    if (nowDict == nil) { //外层id为空，则第一次遇到这个key
        ...
        for (NSString *repeatKey in _repeatKeys) {
            if ([repeatKey isKindOfClass:[NSString class]]) {
                if ([key isEqualToString:repeatKey]) { //重复key
        ...
        if ([nowDict isKindOfClass:[NSMutableDictionary class]]) { //第二次遇到这个key，是重复，转换成数组
            ...
            if ([nowDict isKindOfClass:[NSMutableArray class]]) {
                array = (NSMutableArray*)nowDict;
            } else {
                array = [NSMutableArray arrayWithObject:nowDict];
            }
        }
...
{
   return [ZParserXML parserXMLData:Data];
}

NSDictionary* ZParserXMLData(NSData *Data, NSArray *repeatKeys)
{
    return [ZParserXML parserXMLData:Data repeatKey:repeatKeys];
}

NSDictionary* ZParserXMLString(NSString *String)
{
   return [ZParserXML parserXMLString:String];
}
